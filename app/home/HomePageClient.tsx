...
export type ServerHomeSnapshot = {
  generatedAt?: string;
  suggestions?: Movie[];
  latestMovies?: Movie[];
  homeLatestTvSeries?: any[];
  tvSections?: { latest?: any[]; popular?: any[]; featured?: any[] };
} | null;

function hasFullMovieSnapshot(s: ServerHomeSnapshot): boolean {
  return Boolean(
    s &&
      Array.isArray(s.suggestions) &&
      s.suggestions.length > 0 &&
      Array.isArray(s.latestMovies) &&
      s.latestMovies.length > 0
  );
}
...
export default function HomePageClient({
  ...
  serverSnapshot: ServerHomeSnapshot;
}) {
  const snapOk = hasFullMovieSnapshot(serverSnapshot);
  const limitInit = 16;
  const tsInit = serverSnapshot?.tvSections;
  const tvSectionsOk = Boolean(
    tsInit &&
      ((tsInit.latest?.length ?? 0) > 0 ||
        (tsInit.popular?.length ?? 0) > 0 ||
        (tsInit.featured?.length ?? 0) > 0)
  );

  const [categories, setCategories] = useState<{[key: string]: Movie[]}>({});
  const [loading, setLoading] = useState(!snapOk);
  const [activeCategory, setActiveCategory] = useState("Suggestions");
  const [allMovies, setAllMovies] = useState<Movie[]>([]);
  /** Homepage movies: server snapshot or API/TMDB. */
  const [homeSuggestionMovies, setHomeSuggestionMovies] = useState<HomeMovieCard[]>(() =>
    snapOk
      ? serverSnapshot!.suggestions!.slice(0, limitInit).map((m) => ({
          movie: m as Movie,
  ...
  const [homeLatestMovies, setHomeLatestMovies] = useState<HomeMovieCard[]>(() =>
    snapOk
      ? serverSnapshot!.latestMovies!.slice(0, limitInit).map((m) => ({
          movie: m as Movie,
          source: "fallback" as const,
        }))
      : []
  ...
  const [homeLatestTvSeries, setHomeLatestTvSeries] = useState<any[]>(() => {
    const tv = serverSnapshot?.homeLatestTvSeries;
    return Array.isArray(tv) && tv.length > 0 ? tv.slice(0, limitInit) : [];
  });
  const [suggestionsReady, setSuggestionsReady] = useState(snapOk);
  const [latestMoviesReady, setLatestMoviesReady] = useState(snapOk);
  const [latestTvReady, setLatestTvReady] = useState(snapOk);
  ...
  const [latestTvSeries, setLatestTvSeries] = useState<any[]>(() =>
    tvSectionsOk && Array.isArray(tsInit?.latest) ? tsInit!.latest!.slice(0, limitInit) : []
  );
  const [popularTvSeries, setPopularTvSeries] = useState<any[]>(() =>
    tvSectionsOk && Array.isArray(tsInit?.popular) ? tsInit!.popular!.slice(0, limitInit) : []
  );
  const [featuredTvSeries, setFeaturedTvSeries] = useState<any[]>(() =>
    tvSectionsOk && Array.isArray(tsInit?.featured) ? tsInit!.featured!.slice(0, limitInit) : []
  );
  /** No fetch when snapshot already has TV sections, or snapshot movies-only (skip TV API for 0-request). */
  const [tvHomeLoading, setTvHomeLoading] = useState(() => !tvSectionsOk && !snapOk);
  ...
  useEffect(() => {
    ...
    if (hasFullMovieSnapshot(serverSnapshot)) {
  ...
  }, [DISPLAY_COUNT, serverSnapshot]);

  // Load TV homepage sections once so movie + tv mode latest list stays consistent.
  useEffect(() => {
    if (didLoadTvSectionsRef.current) return;
    const tsnap = serverSnapshot?.tvSections;
    const tvFromSnap =
      tsnap &&
      ((tsnap.latest?.length ?? 0) +
        (tsnap.popular?.length ?? 0) +
        (tsnap.featured?.length ?? 0) >
        0);
    if (tvFromSnap) {
