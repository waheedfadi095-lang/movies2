...
export default function CountryPage({ params }: CountryPageProps) {
  ...
  const loadCountryMovies = async () => {
    try {
      ...
      // Load first 7 movies for this country
      const countryIndex = getCountryIndex(slugParam);
      const startIndex = 7000 + (countryIndex * 300);
  ...
  const loadMoreMovies = async () => {
    ...
    setLoadingMore(true);
    try {
      const countryIndex = getCountryIndex(slug);
      const startIndex = 7000 + (countryIndex * 300) + (currentPage * 7);
      const endIndex = startIndex + 7;
      const movieIds = BULK_MOVIE_IDS.slice(startIndex, endIndex);
